...
def generate_confusion_matrix_from_file(path: str, title: str = 'Confusion Matrix'):
    # Load JSON data
    with open(path, 'r') as f:
        json_obj = json.load(f)
    
    # Extract true and predicted labels
    y_true = json_obj['Results Testset']['y_test']
    y_pred = json_obj['Results Testset']['y_hat_test']
    ...
    ax.set_xticks([i + 0.5 for i in range(len(class_labels))])
    ax.set_yticks([i + 0.5 for i in range(len(class_labels))])
    ax.set_xticklabels(class_labels, rotation=45, ha='right')
    ax.set_yticklabels(class_labels, rotation=0)


    # Construct output path ({id}_cm.png)
    # For example, if your file is 'experiment_results.json', this will produce 'experiment_results_cm.png'
    out_path = path.replace('.json', '_cm.png')
...
def generate_training_combined_plot_from_file(path: str, title):
    """Produce a combined loss and accuracy graph of a provided experiment result file."""
    ...
    eval_line, = ax2.plot(range(1, len(eval_accuracy) + 1), eval_accuracy, linestyle='-', label='Eval Accuracy')
    train_line, = ax2.plot(range(1, len(train_accuracy) + 1), train_accuracy, linestyle='-', label='Train Accuracy')
    ax2.tick_params(axis='y')

    # Create a second y-axis for accuracy
    ax1 = ax2.twinx()
    ax1.set_ylabel(json_obj['Config']['loss_func'])
    loss_line, = ax1.plot(range(1, len(loss) + 1), loss, label='Loss', color='red', linestyle='--')
    ...
    # Add legends
    lines = [loss_line, eval_line, train_line]
    ...
    # Save the plot
    output_path = path.replace('.json', '_combined_plot.jpg')
    plt.tight_layout()
...
if __name__ == "__main__":
    path ='/home/skage/projects/ikt450_deep-neural-networks/stocknet-project-ikt450/skage/results/model_RNN_1_FC_2_dataset_TwitterSentimentVolumePriceXPriceY/0003.json'
    generate_confusion_matrix_from_file(path, 
                                        title='Confusion Matrix Shallow Two-Experts RNN')


    generate_training_combined_plot_from_file(path,
                                              title='Loss and Accuracy - Shallow Deep Two-Experts RNN')