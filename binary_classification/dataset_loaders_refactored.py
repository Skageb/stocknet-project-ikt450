...
class TwitterSentimentVolumePriceXPriceY(Dataset):
    ...
    def __init__(self, start_date, end_date, day_lag:int, tweets_per_day:int, words_per_tweet:int, twitter_root:str, price_root:str, **kwargs) -> None:
        ...
        self.scalers['Volume'].fit(np.array(self.all_price_values[self.dataset_cols.index('Volume')-2]).reshape(-1, 1))
        print(f'Normalization fitting done for feature Volume')
        #print(len(self.all_movement_percents), len(self.all_volumes), len(self.all_sentiments))
    ...
    def __prepare_dataset__(self):
        ...
        self.train_data_samples = collect_stock_date_pairs(self.train_start_date, self.train_end_date, self.day_lag, self.price_dir)
        ...
        #Extract all feature values for normilization
        self.all_price_values = get_all_values_price(self.dataset_cols[self.n_twitter_features:], self.train_data_samples, self.price_dir)
    ...
    def __getitem__(self, idx):
        ...
        for idx, prior_date in enumerate(dates_in_lag):
            ...
            if not row.empty:
                ...
                price_features[-1] = np.float64(self.scalers['Volume'].transform(np.array(price_features[-1]).reshape(-1, 1)).flatten()[0])
            else:
                # Handle missing data by inserting zeros or previous valid data
                price_features = [0, 0, 0, 0, 0, 0]
            if len(price_features) != self.n_price_features:
        ...
        #print(twitter_sequence.shape, price_sequence.shape, x.shape)
...
if __name__ == '__main__':
    ...
    **dataset_loader_args
    ...
    y_train = debug_dataset(train_dataset)

    print('Debugging Evalset')
    y_eval = debug_dataset(eval_dataset)

    print('Debugging Testset')
    y_test = debug_dataset(test_dataset)
    ...
    train_dist = label_distribution(y_train)
    eval_dist = label_distribution(y_eval)
    test_dist = label_distribution(y_test)

    # Labels for the pie charts
    labels = ['Up', 'Down']

    # Define colors for the pie charts
    colors = ['#BDECB6', '#F7C7C7']

    # Create the PDF
    pdf_file = PdfPages('label_distribution_pie_charts.pdf')

    plt.figure(figsize=(12, 4))

    # Train set pie chart
    plt.subplot(1, 3, 1)
    plt.pie(train_dist, labels=labels, autopct='%1.1f%%', colors=colors)
    plt.title('Train Set')

    # Eval set pie chart
    plt.subplot(1, 3, 2)
    plt.pie(eval_dist, labels=labels, autopct='%1.1f%%', colors=colors)
    plt.title('Evaluation Set')

    # Test set pie chart
    plt.subplot(1, 3, 3)
    plt.pie(test_dist, labels=labels, autopct='%1.1f%%', colors=colors)
    plt.title('Test Set')
    ...
    # Inform user
    file_path = "label_distribution_pie_charts.pdf"
