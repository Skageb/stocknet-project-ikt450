...
class SentimentPriceXPriceY(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #Extract the price_movement label from the given date
        price_movement = price_df.loc[price_df['Date'] == target_date_obj, 'Movement_Percent'].values[0]
        y = 1 if price_movement > 0.0055 else 0
        ...
        #Iterate over last n days of twitter data and extract average sentiment for each day
        for prev_date in get_n_prior_dates(target_date, self.day_lag):
            tweet_file = os.path.join(self.tweet_dir, stock_name, f"{prev_date}.json")
            daily_tweets = []
            ...
            if os.path.exists(tweet_file):
                ...
                for tweet in tweets:
                    ...
                    daily_tweets.append(joined_tweet)

                #Calculate average twitter sentiment of stock tweets
                if daily_tweets:
                    average_sentiment = np.mean(self.get_sentiment_from_tweets(daily_tweets))

                #If no tweets use the prior days sentiment or neutral(3) if this is the first day of the lag
                else:
                    if average_daily_sentiment:
                        averga_sentiment = average_daily_sentiment[-1]
                    else:
                        average_sentiment = 3
                average_daily_sentiment.append(average_sentiment)
